...
public class SystemTradingTableInitMenu extends AbstractContainerMenu {
    ...
    public SystemTradingTableInitMenu(int containerId, @NotNull Inventory playerInventory) {
        this(containerId, playerInventory, new ItemStacksResourceHandler(SystemTradingTableBlockEntity.INVENTORY_SIZE), ContainerLevelAccess.NULL);
    }

    public SystemTradingTableInitMenu(int containerId, @NotNull Inventory playerInventory, @NotNull ItemStacksResourceHandler inventory, @NotNull ContainerLevelAccess access) {
        super(TTMenuTypes.SYSTEM_TRADING_TABLE_INIT.get(), containerId);
        this.access = access;
        ...
        return this.access.evaluate((level, pos) -> {
            ...
            if (id == BUTTON_INITIALIZE) {
                if (table.isInitialized()) {
                    player.sendOverlayMessage(Component.translatable("message.trading_table.init_already_done"));
                    return false;
                }
                ...
                if (table.finalizeInitialization(player)) {
                    this.pendingTableName.setLength(0);
                    this.pendingNameHighByte = -1;
                    player.sendOverlayMessage(Component.translatable("message.trading_table.init_success"));
                    return true;
                }
            ...
            if (table.isInitialized()) {
                player.sendOverlayMessage(Component.translatable("message.trading_table.init_locked"));
                return false;
            }

            if (id == BUTTON_TOGGLE_TYPE) {
                table.setBuyOrder(!table.isBuyOrder());
                player.sendOverlayMessage(Component.translatable("message.trading_table.toggled_trade_type"));
                return true;
            }
            ...
            if (id == BUTTON_MIN_PLUS || id == BUTTON_MIN_PLUS_8 || id == BUTTON_MIN_PLUS_32) {
                ...
                player.sendOverlayMessage(Component.translatable("message.trading_table.updated_min_trade"));
                return true;
            }
            ...
            if (id == BUTTON_PRICE_PLUS || id == BUTTON_PRICE_PLUS_8 || id == BUTTON_PRICE_PLUS_32) {
                ...
                player.sendOverlayMessage(Component.translatable("message.trading_table.updated_price"));
                return true;
            }
